 ...
 * @since 1.0.0
 */
class DICM_Reviews extends ET_Builder_Module {
	// Module slug (also used as shortcode tag)
	public $slug       = 'dicm_reviews';
	...
	function init() {
		// Module name
		$this->name             = esc_html__( 'Reviews', 'dicm-divi-custom-modules' );
		...
		$this->settings_modal_toggles  = array(
			'general'  => array(
				'toggles' => array(
					'reviews_main_content' => esc_html__( 'Reviews', 'dicm-divi-custom-modules' )
				),
			),
		);
	}
	...
	function get_fields() {
		return array(
			'title' => array(
				...
				'toggle_slug'     => 'reviews_main_content',
			),
			'yelp_id' => array(
				'label'           => esc_html__( 'Yelp ID', 'dicm-divi-custom-modules' ),
				'type'            => 'text',
				'option_category' => 'basic_option',
				'description'     => esc_html__( 'Enter the business\'s Yelp ID (located after "/biz/" in the address bar when you visit their Yelp page).', 'dicm-divi-custom-modules' ),
				'toggle_slug'     => 'reviews_main_content',
			)
		);
	}
	...
	function render( $attrs, $content = null, $render_slug ) {
		...
		$output = sprintf('
			<div class="dicm-title"><h3>%1$s</h3></div>
			<div class="dicm-content" data-yelp-id="%2$s"></div>
			',
			$title,
			$yelp_id
		);
